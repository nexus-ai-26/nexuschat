...
@pytest.mark.parametrize("model_name", ["cohere:test", "mistral:mistral-large-latest"])
def test_unmanaged_provider_passes_through_without_a_key(model_name: str):
    # pydantic-ai owns credential resolution for providers we don't manage.
    settings = build(model_name=model_name)
...
def test_default_model_is_openai():
    ...
    assert settings.llm_provider_order == (
        "openai,deepseek,gemini,kimi,openrouter,nvidia,groq"
    )
